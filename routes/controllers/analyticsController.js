...
router.post('/profit', async (req, res) => {
    try {
        ...
        averageUsdtProfitPercentage = usdtProfitPercentage / btcuser.length;
        let usdtResult = {
...
router.post('/monthlyprofit', async (req, res) => {
    try {
        var totalBtcProfit = 0;
        var minBtcProfit = 100000000;
        var maxBtcProfit = -100000000;
        ...
        var totalBtcFundsInvested = 0;
        var averageBtcProfitPercentage = 0
        let btcuser = await btcTrades.find({ email: req.body.email, status: 2, pair_type: "BTC", $and: [{ created_at: { $gt: 1572653505994 } }, { created_at: { $lt: 1575048683000 } }] });
        for (data of btcuser) {
            if (data.sell_price !== null && data.buy_price !== null && data.quantity !== null) {
                var profit = parseFloat(data.sell_price) - parseFloat(data.buy_price);
                if (profit <= minBtcProfit) {
        ...
        var totalUsdtFundsInvested = 0;
        var averageUsdtProfitPercentage = 0;
        let ussdtuser = await btcTrades.find({ email: req.body.email, status: 2, pair_type: "USDT", $and: [{ created_at: { $gt: 1572653505994 } }, { created_at: { $lt: 1575048683000 } }] });
        for (data of ussdtuser) {
            if (data.sell_price !== null && data.buy_price !== null && data.quantity !== null) {
                var profit = parseFloat(data.sell_price) - parseFloat(data.buy_price);
                if (profit <= minUsdtProfit) {
